function [total_grid, new_grid] = timeFusion( obj_new_grid, obj, changes)

dempster_normalization = 0;

% if size(changes,1) > size(changes,2)
%     count_changes = size(changes,1);
% else
%     count_changes = size(changes,2);
% end
total_grid=obj.X; obj.X=[];
...
for i = 1:size(changes,1)
    ...
    new_grid(col,row,:) = [0,0,0,1,0];
    m1(3) = 0.0;
    m2(3) = 0.0;
    %         [free, occ, conf, unk] = conjunctive(m1,m2);
    if (m1(4) ~= 1.0 && m2(4) ~= 1.0) || ~isequal(m1,m2)
%         [free, occ, conf, unk] = conjunctiveMobile(m1,m2);
%         [free, occ, conf, unk] = conjunctive(m1,m2);
        free = m1(4) * m2(1) + m1(1) * m2(1) + m1(1) * m2(4); 
        occ = m1(4) * m2(2) + m1(2) * m2(2) + m1(2) * m2(4); 
        conf = m1(3) * m2(1) + m1(3) * m2(2) + m1(3) * m2(3) + m1(3) * m2(4) + m2(3) * m1(1) + m2(3) * m1(2) + m2(3) * m1(3) + m1(1) * m2(2) + m1(2) * m2(1);
        unk = m1(4) * m2(4);
