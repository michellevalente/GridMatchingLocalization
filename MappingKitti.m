...
[lidar_grid, aging_lidar, changes_lidar, limits,position] = lidarMappingKitti(velo, current_pose, origin, size_grid_x, ...
    size_grid_y, resolution, obj_aging, obj_grid_lidar, frame, Tr_velo_to_cam);

%% Add Free space
[yaw, pitch, roll]  = dcm2angle(current_pose(1:3,1:3));
theta = 0 + radtodeg(yaw);
Tr_new_velo = [1 0 0 10; 0 1 0 0; 0 1 0 0; 0 0 0 1];
pose_new_velo =    Tr_new_velo * current_pose  ;
...
% obj_changes=Reference; obj_changes.X=changes; clear changes;
...
%     no_fusion = 0;
    ...
    if (abs(round(d,1)) > 3.0) || round(r,2) > 0.5 || abs(round(dx,1)) > 1.8 || converged == 0
...
[total_grid, aging_lidar,aging_stereo] = perceptionGridKitti( obj_total_grid, obj_aging_stereo, obj_aging_lidar,frame,limits);  
